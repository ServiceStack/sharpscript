...
Which will host your App at `http://localhost:5000` which in `debug` mode will enable [hot reloading](http://sharpscript.net/docs/hot-reloading) 
...
One benefit of creating your APIs with C# ServiceStack Services instead of [API Pages](http://sharpscript.net/docs/sharp-apis) is that you can generate TypeScript DTOs with:
...
Then to deploy Sharp Apps you just need to copy the `/app` and `/web` folders to any server with .NET Core 2.1 runtime installed. 
